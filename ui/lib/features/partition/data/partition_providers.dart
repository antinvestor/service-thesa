...
final tenantsProvider = FutureProvider<List<TenantObject>>((ref) async {
  final repo = await ref.watch(partitionRepositoryProvider.future);
  return repo.listTenants();
});

/// Single tenant detail.
final tenantDetailProvider = FutureProvider.family<TenantObject, String>((
  ref,
  tenantId,
) async {
  final repo = await ref.watch(partitionRepositoryProvider.future);
...
final partitionsProvider = FutureProvider<List<PartitionObject>>((ref) async {
  final repo = await ref.watch(partitionRepositoryProvider.future);
  return repo.listPartitions();
});

/// Single partition detail.
final partitionDetailProvider = FutureProvider.family<PartitionObject, String>((
  ref,
  partitionId,
) async {
  final repo = await ref.watch(partitionRepositoryProvider.future);
...
final partitionRolesProvider = FutureProvider<List<PartitionRoleObject>>((
  ref,
) async {
  final repo = await ref.watch(partitionRepositoryProvider.future);
...
final pagesProvider = FutureProvider<List<PageObject>>((ref) async {
  final repo = await ref.watch(partitionRepositoryProvider.future);
...
final pagesForPartitionProvider =
    FutureProvider.family<List<PageObject>, String>((ref, partitionId) async {
      final repo = await ref.watch(partitionRepositoryProvider.future);
      return repo.listPages(partitionId: partitionId);
    });

/// Access list provider (all access grants).
final accessListProvider = FutureProvider<List<AccessObject>>((ref) async {
  final repo = await ref.watch(partitionRepositoryProvider.future);
...
final accessForPartitionProvider =
    FutureProvider.family<List<AccessObject>, String>((ref, partitionId) async {
      final repo = await ref.watch(partitionRepositoryProvider.future);
      return repo.listAccess(partitionId: partitionId);
    });

/// Access roles list provider (all).
final accessRolesProvider = FutureProvider<List<AccessRoleObject>>((ref) async {
  final repo = await ref.watch(partitionRepositoryProvider.future);
...
final clientsForPartitionProvider =
    FutureProvider.family<List<ClientObject>, String>((ref, partitionId) async {
      final repo = await ref.watch(partitionRepositoryProvider.future);
      return repo.listClients(partitionId: partitionId);
    });

/// Registered service namespaces and their permissions.
final serviceNamespacesProvider = FutureProvider<List<ServiceNamespaceObject>>((
  ref,
) async {
  final repo = await ref.watch(partitionRepositoryProvider.future);
